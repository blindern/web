...
<p>
	...
	<pre>'.htmlspecialchars_utf8($query).'</pre>
</p>
<table cellpadding="2" cellspacing="0" border="1" frame="hidden" rules="all" class="q_debug">
	<thead>
		...
		while ($field = mysql_fetch_field($result)) {
			echo '
			<th bgcolor="#EEEEEE">'.htmlspecialchars_utf8($field->name).'</th>';
		}
	...
	<tbody>';
		...
		<tr>';
				...
				foreach ($row as $value) {
					echo '
			<td>'.($value == NULL ? '<i style="color: #CCC">NULL</i>' : ($value === "" ? '<i style="color: #CCC">TOMT</i>' : nl2br(htmlspecialchars_utf8($value)))).'</td>';
				}
