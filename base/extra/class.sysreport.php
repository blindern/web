...
class sysreport
{
	 ...
	 */
	public static function log($data, $title = "Email Log")
	{
		global $_base, $up_id;
	...
	public static function exception_format_stack($exception, $html = true, $show_parameters = true)
	{
		...
		foreach ($trace as $row)
		{
			...
			else
			{
				if ($html)
				{
					$row['file'] = htmlspecialchars_utf8($row['file']);
					if (isset($row['class']))
					{
						$row['class'] = htmlspecialchars_utf8($row['class']);
						$row['type'] = htmlspecialchars_utf8($row['type']);
					}
					$row['function'] = htmlspecialchars_utf8($row['function']);
				}
				...
				if (isset($row['args']) && count($row['args']) > 0)
				{
					...
					if ($html) $args = htmlspecialchars_utf8($args);
					@ob_clean();
...
'.$stack_text;
		...
		self::log($text, "Fanget Exception".($subtitle == "" ?  "" : " [$subtitle]"));
	}
	...
	public static function exception_handler(Exception $exception)
	{
		...
		$html = '
<p><b>Fatal feil:</b> Uoppfanget unntak &laquo;'.htmlspecialchars_utf8(get_class($exception)).'&raquo;.</p>
<p><b>Kastet i</b> '.htmlspecialchars_utf8($exception->getFile()).' <b>på linje</b> '.$exception->getLine().'.</p>
<div style="margin: 1em 0; border: 2px solid #BBB; padding: 5px; background-color: #EEE"><b>Melding:</b> <pre>'.htmlspecialchars_utf8($exception->getMessage()).'</pre></div>';
		...
		if (!MAIN_SERVER)
		{
			...
			$user = htmlspecialchars_utf8(DBUSER);
			$pass = htmlspecialchars_utf8(DBPASS);
...
'.$stack_html.'
<p>_GET:</p>
<pre>'.htmlspecialchars_utf8(print_r($_GET, true)).'</pre>
<p>_POST:</p>
<pre>'.htmlspecialchars_utf8(print_r($_POST, true)).'</pre>
<p>_COOKIE:</p>
<pre>'.htmlspecialchars_utf8(print_r($_COOKIE, true)).'</pre>'.(isset($_SESSION) ? '
<p>_SESSION:</p>
<pre>'.htmlspecialchars_utf8(print_r($_SESSION, true)).'</pre>' : '');
		}
...
'.mysql_result($result, 0, "Status");
				...
				if (!MAIN_SERVER)
				{
					$html .= '
<p>InnoDB status:</p>
<pre>'.htmlspecialchars_utf8(mysql_result($result, 0, "Status")).'</pre>';
				}
			}
		}
		
		// send e-post og logg
		if (!self::log($text, "Exception"))
		{
