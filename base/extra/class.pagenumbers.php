...
class pagenumbers
{
	...
	{
		$this->page_1 = $page_1;
		$this->page_1_safe = htmlspecialchars_utf8($this->page_1);
		$this->page_x = $page_x;
		$this->page_x_safe = htmlspecialchars_utf8($this->page_x);
		$this->page = (int) $page;
	...
	public function build()
	{
		...
		for ($i = 1; $i <= $this->pages; $i++)
		{
			...
			if ($i > 1 && $i < $this->pages && ($i < $low || $i > $high))
			{
				...
				// hopp kun over hvis det er mer enn �n side
				if ($n - $i >= 1)
				{
					$i = $n;
					$ret[] = '<a href="#" onclick="var value=parseInt(prompt(\'�nsket sidetall? (1 til '.$this->pages.')\','.$this->page.'));if(value>1&&value<'.$this->pages.'){setTimeout(function(){window.location=unescape(\''.rawurlencode($this->page_x).'\').replace(/_pageid_/g,value)},1)}else if(value==1){setTimeout(function(){window.location=unescape(\''.rawurlencode($this->page_1).'\')},1)}return false">..</a>';
					continue;
				}
			}
