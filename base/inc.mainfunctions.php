 ...
 */
function share_url($share_url = null)
{
	if (!$share_url)
	{
		$share_url = $_SERVER['REQUEST_URI'];
		if (($pos = strpos($share_url, "?")) !== false) $share_url = substr($share_url, 0, $pos);
	}
	else $share_url = fix_path($share_url);
	
	// benytte en annen adresse?
	static $map = array();
	$map[ess::$s['rpath']."/revy"]                                     = "/2011/nyhetsarkiv/68-blindernrevyen-2011";
	$map[ess::$s['rpath']."/nyhetsarkiv/facebook-og-twitter"]          = "/2011/nyhetsarkiv/1-facebook-og-twitter";
	$map[ess::$s['rpath']."/nyhetsarkiv/tilbakeblikk-uka09"]           = "/2011/nyhetsarkiv/46-tilbakeblikk-uka09";
	$map[ess::$s['rpath']."/nyhetsarkiv/intro-uka"]                    = "/2011/nyhetsarkiv/48-intro-uka";
	$map[ess::$s['rpath']."/nyhetsarkiv/vil-du-samarbeide-med-uka-pa-blindern"] = "/2011/nyhetsarkiv/50-vil-du-samarbeide-med-uka-pa-blindern";
	$map[ess::$s['rpath']."/nyhetsarkiv/stjernespekket-revysus"]       = "/2011/nyhetsarkiv/66-stjernespekket-revysus";
	$map[ess::$s['rpath']."/nyhetsarkiv/datarock"]                     = "/2011/nyhetsarkiv/69-datarock";
	...
	return "http://blindernuka.no".$share_url;
}

/**
 * Sett inn share-boks for Facebook
 */
function share_box($share_url = null)
{
	$share_url = share_url($share_url);
	
	return '
	<div class="article-share">
		<iframe src="'.share_box_src($share_url).'" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:450px; height:25px;" allowTransparency="true"></iframe>
	</div>';
}

function share_box_src($url)
{
	return 'http://www.facebook.com/plugins/like.php?href='.urlencode($url).'&amp;layout=standard&amp;show_faces=false&amp;width=450&amp;action=like&amp;colorscheme=light&amp;height=80';
}


/**
 * Generer h2-tag for sider
 */
function generate_page_title($title)
{
	return htmlspecialchars_utf8(mb_strtoupper($title));
	
	return '
	<img src="'.ess::$s['rpath'].'/draw/draw.php?text='.urlencode('{s18w3c000000}'.mb_strtoupper($title)).'" alt="'.htmlspecialchars_utf8($title).'" />';
}
 ...
 * Formatter beløp
 */
function format_nok($amount, $with_nok = null)
{
	$n = round($amount, 2);
	$dec = round($n) != $n;
	
	$n = number_format($amount, $with_nok || $dec ? 2 : 0, ",", " ");
	
	if ($with_nok) return "NOK ".$n;
	return "kr $n".($dec ? "" : ",-");
}


function get_hidden_fields($vars, $prefix = "", $suffix = "")
{
	$content = '';
	foreach ($vars as $name => $value)
	{
		...
		{
			$content .= '
	<input type="hidden" name="'.htmlspecialchars_utf8($prefix.$name.$suffix).'" value="'.htmlspecialchars_utf8($value).'" />';
		}
		
		elseif (is_array($value))
		{
			$content .= get_hidden_fields($value, $prefix.$name.$suffix."[", "]");
		}
	}
